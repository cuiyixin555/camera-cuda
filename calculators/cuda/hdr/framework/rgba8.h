...
public:
  ...
  explicit RGBA8(std::uint32_t color) : color(color) {}
  ...
  template <int i> friend std::uint8_t channel(const RGBA8 &color);
};

template <int i> inline std::uint8_t channel(const RGBA8 &color) {
