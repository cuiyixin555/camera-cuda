...
def _requires_rules_are_enabled():
    return select({
        "@camera_cuda//cuda:is_enabled": [],
        "//conditions:default": ["@camera_cuda//cuda:rules_are_enabled"],
    })

def _requires_valid_toolchain_is_configured():
    return select({
        "@camera_cuda//cuda:is_valid_toolchain_found": [],
        "//conditions:default": ["@camera_cuda//cuda:valid_toolchain_is_configured"],
    })
